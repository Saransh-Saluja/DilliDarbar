...
// Updated MenuImages component with scroll animations
const MenuImages = ({ menuImages, className = "" }) => {
  const [isVisible, setIsVisible] = useState(false);
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setTimeout(() => {
            setIsVisible(true);
          }, 200); // Small delay for the images
        }
      },
      {
        threshold: 0.1,
  ...
  return (
    <div ref={imagesRef} className={`flex-shrink-0 hidden lg:block ${className}`}>
      <div className="flex flex-col space-y-6">
        {images.length > 0 ? (
          images.map((image, index) => (
            <div 
              key={index} 
              className={`w-64 h-80 bg-gray-800 border border-gray-700 rounded-lg shadow-lg overflow-hidden flex items-center justify-center transition-all duration-800 ease-out ${
          ...
          <div className={`w-64 h-80 bg-gray-800 border border-gray-700 rounded-lg shadow-lg overflow-hidden flex items-center justify-center transition-all duration-800 ease-out ${
            isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
          }`}>
            <div className="text-center p-8">
              <div className="w-16 h-16 bg-amber-600 rounded-full mx-auto mb-4 flex items-center justify-center">
...
// Two Column Menu Component with scroll animations
const TwoColumnMenu = ({ sectionHeading, leftColumn, rightColumn, menuImages, title, subtitle }) => {
